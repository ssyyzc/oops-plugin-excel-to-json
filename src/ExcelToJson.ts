 ...
 */
async function convert(src: string, dst: string, name: string, isClient: boolean, dir : string) {
    // let r: any = {};
    // let names: any[] = [];          // 文名字段名
    // let keys: any[] = [];           // 字段名
...
export function run() {
    ...
    files.forEach((f: any) => {
        ...
        if (ext == "xlsx") {
            if (outJsonPathServer) convert(inputExcelPath + f, outJsonPathServer + name + ".json", name, false, outJsonPathServer);                  // 服务器数据
